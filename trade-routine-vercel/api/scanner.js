...
function getDailyChangeFromShortResult(result, ticker) {
  const closes = getCloses(result);

  if (closes.length < 2) {
    throw new Error(`Not enough short-term data for ${ticker}`);
  }
  ...
  if (typeof last !== 'number' || typeof prev !== 'number' || prev === 0) {
    throw new Error(`Invalid short-term pricing for ${ticker}`);
  }
...
function noteFor(changePct, avgRSI, fromHighPct) {
  ...
  if (fromHighPct !== null && fromHighPct <= -15) bits.push('Well off recent highs.');

  return bits.join(' ') || 'No major signal.';
}

async function getScanRow(ticker) {
  // Long-term: RSI + 52-week high
  const longResult = await fetchYahooChart(ticker, '1y', '1d');
  const longCloses = getCloses(longResult);
  const lastLong = longCloses[longCloses.length - 1];

  if (longCloses.length < 20 || typeof lastLong !== 'number') {
    throw new Error(`Not enough data for ${ticker}`);
  }

  // Short-term: true daily CHG%
  const shortResult = await fetchYahooChart(ticker, '5d', '1d');
  const { last, changePct } = getDailyChangeFromShortResult(shortResult, ticker);
  ...
  const fromHighPct = high52 ? ((lastLong - high52) / high52) * 100 : null;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!tickers.length) {
      return json(res, 400, { error: 'No tickers provided' });
    }
