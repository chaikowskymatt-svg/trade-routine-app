...
async function fetchYahooChart(symbol, range = '1y', interval = '1d'){
  ...
  if (!res.ok) throw new Error(`Yahoo request failed for ${symbol}`);
...
async function getScanRow(ticker){
  ...
  if (closes.length < 20) throw new Error(`Not enough data for ${ticker}`);
  const last = typeof meta.regularMarketPrice === 'number' ? meta.regularMarketPrice : closes[closes.length - 1];
  const prev = typeof meta.chartPreviousClose === 'number' ? meta.chartPreviousClose : closes[closes.length - 2];
  ...
  try {
    ...
    const rows = await Promise.all(tickers.map(async (ticker) => {
      try {
        ...
        return { symbol: ticker, price: null, changePct: 0, avgRSI: 0, fromHighPct: null, signal: 'ERROR', flags: ['WATCH'], note: error.message };
      }
    }));
