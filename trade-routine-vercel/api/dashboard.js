...
async function getSymbolData(label, yahooSymbol){
  ...
  if (closes.length < 20 || typeof last !== 'number') throw new Error(`Not enough price history for ${label}`);

  const rawChangePct = prev ? ((last - prev) / prev) * 100 : 0;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const entries = await Promise.all(Object.entries(symbols).map(async ([label, symbol]) => {
    ...
    return json(res, 200, {
      ...
      econCalendar: {
        wired: false,
        message: 'Economic calendar API not wired yet in this fast version',
        events: [],
      },
      source: 'yahoo_chart_only',
